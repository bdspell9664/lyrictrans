...
class AIService {
            ...
            try {
                ...
                if (!response.ok) {
                    if (retry < maxRetries - 1) {
                        console.log(`百度翻译API请求失败，正在重试 (${retry + 1}/${maxRetries})...`);
                        // 等待一段时间后重试
                ...
                let translatedText = '';
                if (data.trans_result && Array.isArray(data.trans_result)) {
                    translatedText = data.trans_result.map(item => item.dst).join('\n');
                }

                // 如果翻译结果为空或不完整，使用模拟翻译
                if (!translatedText || translatedText.length < text.length * 0.5) {
                    console.log('翻译结果不完整，使用模拟翻译');
                    return this.mockTranslate(text, targetLang, sourceLang);
                }
    ...
    mockTranslate(text, targetLang, sourceLang = 'auto') {
        ...
        const mockTranslations = {
            ...
            '圧倒的で感動的な': '压倒性的感动的',
            '理想的超えて完璧な': '超越理想的完美的',
            'お前なんかいてもいなくても': '你在不在都一样',
            '意地悪いこの世界の口癖で': '用这个世界擅长的坏心眼口头禅',
            '僕の耳元で飽きもせずに': '在我耳边不厌其烦地',
            '話がしたいなら顔を見せなよ': '想说话的话就露出脸吧',
            '今すぐさ': '现在马上',
            'あと何万回の後悔で': '还有几万次的后悔',
            '僕は僕の期待を超えられるだろう': '我能超越自己的期待吗',
            'この心に足が生えてたら': '如果这颗心长了脚',
            '今日の行き先は違っていたかな': '今天的目的地会不同吗',
            '就職試験の合格通知': '求职考试的合格通知',
            '面白い人間の不合格通知': '有趣人类的不合格通知',
            '心は彼方': '心在远方',
            '全力疾走で': '全力奔跑',
            'もういないだろう': '已经不在了吧',
            '俺に用はないだろう': '我没什么用吧',
            ...
            '無安打無失点の未来': '无安打无失分的未来',
            '近頃いつ僕は僕のことを': '最近我什么时候',
            '驚かせてやってあげたかな': '让自己惊讶了呢',
            '逃げられてしまう前に早いとこ': '在逃跑之前赶紧',
            '一生お前についていくって言わせてやる': '让你说一辈子跟着你',
            '自分で自分予測変換': '自己进行自己的预测变换',
            '説明書などなしで充分だって': '没有说明书也足够',
            'じゃあどこのページに書いてあった？': '那写在第几页了？',
            'しょうがないだろう': '没办法吧',
            '無敵的超えて完璧な': '超越无敌的完美的',
            '創造的で本能的な': '创造性的本能的',
            '前人未到的な 世界': '前所未有的世界',
        ...
        translated = lines.map(line => {
            ...
            for (const [key, value] of sortedTranslations) {
                lineTranslated = lineTranslated.replace(new RegExp(key, 'g'), value);
            }
            
            // 对于日语，添加简单的字符级翻译标记
            if (sourceLang === 'ja' || /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF]/.test(line)) {
                // 如果还有未翻译的日语字符，添加简单的翻译标记
                if (lineTranslated === line) {
                    return line + ' [译]';
                }
            }
