...
class AIService {
    ...
    async translateWithBaidu(text, targetLang, sourceLang = 'auto') {
        ...
        try {
            // 先检查代理服务器是否可用
            try {
                const proxyCheckResponse = await fetch('http://localhost:3001', { method: 'HEAD' });
                if (!proxyCheckResponse.ok) {
                    throw new Error('代理服务器不可用');
                }
            } catch (proxyError) {
                console.warn('代理服务器不可用，尝试直接请求百度API:', proxyError.message);
                // 如果代理不可用，回退到直接请求百度API
                return this.directBaiduRequest(text, from, to, appid, salt, sign);
            }
